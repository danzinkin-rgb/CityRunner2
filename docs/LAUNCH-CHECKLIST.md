...
| 16 | Game Center leaderboards + achievements | **DONE** | Native GameKit plugin (ios/App/App/GameCenterPlugin.swift, hand-rolled, no third-party pod — see COMPLIANCE §4), registered by hand in ios/App/App/BridgeViewController.swift since Capacitor 7 only auto-registers plugins shipped as real npm packages. Verified end-to-end on a real device: authentication, leaderboard score submission, and achievement reporting all confirmed working (a VPN active on the device silently broke achievement reporting only — resolved by turning it off; not a code issue). All 6 leaderboards and all 9 achievements created and localized in App Store Connect, ids match src/core/gamecenter.js. Achievement icons are still placeholder art (flat colour + initials) — swap for real artwork before submission, no code change needed since GameKit serves the icon from ASC. Game Center capability confirmed enabled in Signing & Capabilities |
